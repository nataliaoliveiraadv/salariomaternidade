...
import {
  INK, INK_LIGHT, PAPER, CARD, ROSE, ROSE_DEEP, SAGE, AMBER, CRIMSON, SLATE, BORDER,
  FONT_IMPORT, btnPrimary, btnGhost, iconBtn,
} from "./theme";
...
const SITU_COLOR = { "Em Análise": AMBER, "Concedido": SAGE, "Negado": CRIMSON, "Finalizado": INK_LIGHT };
  ...
  return (
    <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "18px 20px", display: "flex", flexDirection: "column", gap: 10, minWidth: 0 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ width: 30, height: 30, borderRadius: 9, display: "flex", alignItems: "center", justifyContent: "center", background: `${accent}17` }}>
...
function TextField({ label, style, ...props }) {
  return (
    <label style={{ display: "flex", flexDirection: "column", gap: 5, fontFamily: "Inter, sans-serif" }}>
      ...
      <input {...props} style={{ border: `1px solid ${BORDER}`, borderRadius: 8, padding: "8px 10px", fontSize: 13.5, fontFamily: "Inter, sans-serif", color: INK, outline: "none", background: "#FCFBF9", ...style }} />
    </label>
  );
}
function SelectField({ label, options, ...props }) {
  return (
    <label style={{ display: "flex", flexDirection: "column", gap: 5, fontFamily: "Inter, sans-serif" }}>
      <span style={{ fontSize: 12, fontWeight: 600, color: SLATE }}>{label}</span>
      <select {...props} style={{ border: `1px solid ${BORDER}`, borderRadius: 8, padding: "8px 10px", fontSize: 13.5, fontFamily: "Inter, sans-serif", color: INK, outline: "none", background: "#FCFBF9" }}>
...
function Modal({ title, onClose, children, wide }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(28,37,65,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50, padding: 20 }}
      ...
      <div style={{ background: CARD, borderRadius: 16, width: "100%", maxWidth: wide ? 720 : 480, maxHeight: "88vh", overflowY: "auto", boxShadow: "0 20px 60px rgba(28,37,65,0.35)" }}>
        <div style={{ position: "sticky", top: 0, background: CARD, display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 22px", borderBottom: `1px solid ${BORDER}` }}>
          <h3 style={{ fontFamily: "Fraunces, serif", fontSize: 19, fontWeight: 600, color: INK, margin: 0 }}>{title}</h3>
          <button onClick={onClose} style={{ border: "none", background: "transparent", cursor: "pointer", width: 30, height: 30, borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center" }}>
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 22 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.6fr 1fr", gap: 16 }}>
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "18px 20px" }}>
          <h4 style={{ fontFamily: "Fraunces, serif", fontSize: 16, color: INK, margin: "0 0 14px" }}>Concessões por mês</h4>
        ...
        <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "18px 20px" }}>
          <h4 style={{ fontFamily: "Fraunces, serif", fontSize: 16, color: INK, margin: "0 0 14px" }}>Situação da carteira</h4>
          {pieData.length === 0 ? (
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginTop: 8 }}>
            {pieData.map((d) => <Pill key={d.name} color={d.color}>{d.name} · {d.value}</Pill>)}
          </div>
        </div>
      </div>

      <div style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: "18px 20px" }}>
        <h4 style={{ fontFamily: "Fraunces, serif", fontSize: 16, color: INK, margin: "0 0 14px", display: "flex", alignItems: "center", gap: 8 }}>
          <CalendarClock size={17} color={ROSE_DEEP} /> Próximos partos previstos
        </h4>
        {proximosPartos.length === 0 ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {proximosPartos.map((p) => (
              <div key={p.id} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 14px", borderRadius: 10, background: PAPER }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                  <Baby size={16} color={ROSE_DEEP} />
                  <span style={{ fontFamily: "Inter, sans-serif", fontSize: 13.5, fontWeight: 600, color: INK }}>{p.nome}</span>
                  <Pill color={INK_LIGHT}>{p.tipo}</Pill>
                </div>
                <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 12.5, color: ROSE_DEEP, fontWeight: 600 }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 12 }}>
        <div>
          <TextField label="Nome da cliente *" value={f.nome} onChange={set("nome")} style={touched && !nomeValido ? { borderColor: CRIMSON } : undefined} />
          {touched && !nomeValido && <span style={{ fontFamily: "Inter, sans-serif", fontSize: 11.5, color: CRIMSON }}>Informe o nome da cliente para salvar.</span>}
        </div>
      ...
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginTop: 4 }}>
        ...
        <button type="button" onClick={handleSave} disabled={saving} style={{ ...btnPrimary, opacity: saving ? 0.7 : 1 }}>
          {saving ? <Loader2 size={15} className="spin" /> : <Save size={15} />} Salvar registro
        </button>
      </div>
    </div>
  );
...
function ControleBeneficios({ api }) {
  ...
  const remove = (id) => { if (confirm("Excluir este registro? Essa ação não pode ser desfeita.")) api.remove(id); };
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <div style={{ position: "relative" }}>
            ...
            <input placeholder="Buscar por nome" value={busca} onChange={(e) => setBusca(e.target.value)} style={{ border: `1px solid ${BORDER}`, borderRadius: 9, padding: "8px 12px 8px 30px", fontFamily: "Inter, sans-serif", fontSize: 13, minWidth: 200, background: CARD }} />
          </div>
          <select value={filtroSituacao} onChange={(e) => setFiltroSituacao(e.target.value)} style={{ border: `1px solid ${BORDER}`, borderRadius: 9, padding: "8px 12px", fontFamily: "Inter, sans-serif", fontSize: 13, background: CARD }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          ], "controle-de-beneficios.csv")}><Download size={15} /> Exportar CSV</button>
          <button style={btnPrimary} onClick={() => setModal("new")}><Plus size={15} /> Novo registro</button>
        </div>
      </div>

      {api.error && <div style={{ color: CRIMSON, fontSize: 13, fontFamily: "Inter, sans-serif" }}>{api.error}</div>}
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontFamily: "Inter, sans-serif" }}>
            <thead>
              <tr style={{ background: PAPER }}>
                {["Cliente", "Situação", "Fase", "Tipo", "DER", "Concessão", "Benefício", "Honorários", ""].map((h) => (
                  <th key={h} style={{ textAlign: "left", padding: "11px 14px", fontSize: 11.5, fontWeight: 700, color: SLATE, textTransform: "uppercase", letterSpacing: "0.03em", whiteSpace: "nowrap" }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {api.loading && <tr><td colSpan={9} style={{ padding: "26px 14px", textAlign: "center", color: SLATE, fontSize: 13 }}><Loader2 size={14} className="spin" /> Carregando…</td></tr>}
              ...
              {filtrados.map((b) => (
                <tr key={b.id} style={{ borderTop: `1px solid ${BORDER}` }}>
                  ...
                  <td style={{ padding: "11px 14px", fontSize: 12.5, fontFamily: "'IBM Plex Mono', monospace", color: ROSE_DEEP }}>{formatBRL(b.valorHonorarios)}</td>
                  <td style={{ padding: "11px 14px" }}>
                    <div style={{ display: "flex", gap: 6, justifyContent: "flex-end" }}>
                      <button onClick={() => setModal(b)} style={iconBtn}><Pencil size={14} color={SLATE} /></button>
                      <button onClick={() => remove(b.id)} style={iconBtn}><Trash2 size={14} color={CRIMSON} /></button>
                    </div>
                  </td>
                </tr>
              ))}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", justifyContent: "flex-end", gap: 10 }}>
        ...
        ], "partos-futuros.csv")}><Download size={15} /> Exportar CSV</button>
        <button style={btnPrimary} onClick={() => setModal("new")}><Plus size={15} /> Nova cliente</button>
      </div>

      {api.error && <div style={{ color: CRIMSON, fontSize: 13, fontFamily: "Inter, sans-serif" }}>{api.error}</div>}

      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(270px, 1fr))", gap: 14 }}>
        {api.loading && <p style={{ fontFamily: "Inter, sans-serif", fontSize: 13, color: SLATE }}><Loader2 size={14} className="spin" /> Carregando…</p>}
            ...
            <div key={p.id} style={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: 14, padding: 18, display: "flex", flexDirection: "column", gap: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                ...
                <div style={{ display: "flex", gap: 6 }}>
                  <button onClick={() => setModal(p)} style={iconBtn}><Pencil size={13} color={SLATE} /></button>
                  <button onClick={() => remove(p.id)} style={iconBtn}><Trash2 size={13} color={CRIMSON} /></button>
                </div>
              </div>
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                <Pill color={ROSE_DEEP}>{p.tipo}</Pill>
                <Pill color={p.status === "Cliente" ? SAGE : AMBER}>{p.status}</Pill>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: PAPER, borderRadius: 9, padding: "9px 12px", marginTop: 2 }}>
                ...
                <span style={{ fontFamily: "'IBM Plex Mono', monospace", fontSize: 12.5, fontWeight: 600, color: dias >= 0 ? ROSE_DEEP : SLATE }}>
...
function MainApp({ session }) {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: PAPER, fontFamily: "Inter, sans-serif" }}>
      <style>{FONT_IMPORT}</style>
      <style>{`.spin{animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}`}</style>

      <aside style={{ width: 232, background: INK, color: "#fff", display: "flex", flexDirection: "column", padding: "24px 16px", flexShrink: 0, gap: 26 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "0 6px" }}>
          <div style={{ width: 34, height: 34, borderRadius: 9, background: ROSE, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Baby size={18} color="#fff" /></div>
          <div>
        ...
        <nav style={{ display: "flex", flexDirection: "column", gap: 3 }}>
          {NAV.map((n) => {
            ...
            const Icon = n.icon;
            return (
              <button key={n.id} onClick={() => setTab(n.id)} style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", borderRadius: 9, border: "none", cursor: "pointer", textAlign: "left", background: active ? INK_LIGHT : "transparent", color: active ? "#fff" : "#B8C0D6", fontFamily: "Inter, sans-serif", fontSize: 13.5, fontWeight: 600 }}>
                <Icon size={16} />
        ...
        <div style={{ marginTop: "auto", padding: "12px 6px", fontSize: 11, color: "#8895B8", lineHeight: 1.5 }}>
          ...
          <div style={{ marginTop: 8, opacity: 0.8 }}>Jusystem © 2026</div>
        </div>
      </aside>
      ...
      <main style={{ flex: 1, padding: "28px 32px", overflowY: "auto" }}>
        ...
        {loading ? (
          <div style={{ display: "flex", alignItems: "center", gap: 8, color: SLATE, fontSize: 13 }}><Loader2 size={15} className="spin" /> Carregando dados…</div>
        ) : (
          <>
  ...
  if (session === undefined) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: PAPER }}>
        <Loader2 size={20} color={INK} className="spin" />
        <style>{`.spin{animation:spin 1s linear infinite}@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      </div>
    );
  }
