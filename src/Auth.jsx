...
import { INK, ROSE, ROSE_DEEP, PAPER, CARD, SLATE, BORDER, CRIMSON, SAGE, GLOBAL_STYLES, btnPrimary } from "./theme";
...
export default function Auth() {
  ...
  return (
    <div style={{
      ...
      <div role="region" aria-labelledby={headingId} style={{ width: "100%", maxWidth: 380, background: CARD, borderRadius: 16, padding: 32, border: `1px solid ${BORDER}` }}>
        <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 22 }}>
          <div aria-hidden="true" style={{ width: 36, height: 36, borderRadius: 9, background: ROSE, display: "flex", alignItems: "center", justifyContent: "center" }}>
        ...
        <h2 id={headingId} style={{ fontFamily: "Fraunces, serif", fontSize: 20, color: INK, margin: "0 0 6px" }}>
        ...
        <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <label htmlFor={emailId} style={{ display: "flex", flexDirection: "column", gap: 5 }}>
            <span style={{ fontSize: 12, fontWeight: 600, color: SLATE }}>E-mail</span>
            <input id={emailId} type="email" required autoComplete="email" value={email} onChange={(e) => setEmail(e.target.value)}
              style={{ border: `1px solid ${BORDER}`, borderRadius: 8, padding: "9px 11px", fontSize: 13.5 }} />
          </label>
          <label htmlFor={passwordId} style={{ display: "flex", flexDirection: "column", gap: 5 }}>
          ...
          {error && <div role="alert" style={{ fontSize: 12.5, color: CRIMSON }}>{error}</div>}
          {message && <div role="status" style={{ fontSize: 12.5, color: SAGE }}>{message}</div>}
